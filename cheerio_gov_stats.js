...
async function main() {
  try {
    ...
    const batchSize = 5;
    let totalGovPlUrls = 0; // Initialize a counter for 'gov.pl' URLs
        ...
        batch.map(async (hit) => {
          ...
          if (title) {
            ...
            if (govPlResults.length > 0) {
              ...
              totalGovPlUrls += govPlResults.length; // Increment counter
            } else {
    ...
    console.log(`Total 'gov.pl' URLs found: ${totalGovPlUrls}`); // Log the total count
  } catch (error) {
