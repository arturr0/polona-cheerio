...
async function searchOnGoogleGov(query) {
  ...
  try {
    ...
    if (response.status() === 429) {
      console.error('Rate-limited by Google (HTTP 429). Stopping the app.');
...
async function main() {
  try {
    ...
    console.log(`Total number of URLs found: ${totalUrlsFound}`);
  } catch (error) {
